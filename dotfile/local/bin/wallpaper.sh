...
# If a wallpaper was selected, stop mpvpaper and apply the static wallpaper
if [ -n "$SELECTED" ]; then
    # Stop animated wallpaper if it's running
    pkill mpvpaper 2>/dev/null

    # Apply the static wallpaper
    ...
    sh ~/.config/dunst/update-theme.sh >> /tmp/wallpaper_switcher.log 2>&1
else
